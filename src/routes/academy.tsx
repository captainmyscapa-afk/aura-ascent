import { createFileRoute } from "@tanstack/react-router";
import { AppShell } from "@/components/aurum/AppShell";
import { SectionHeading } from "@/components/aurum/SectionHeading";
import { Play, Lock, Sparkles } from "lucide-react";
import yachtImg from "@/assets/hero-yacht.jpg";
import villaImg from "@/assets/eco-villa.jpg";
import jetImg from "@/assets/eco-jet.jpg";
import carImg from "@/assets/eco-car.jpg";

export const Route = createFileRoute("/academy")({
  component: Academy,
});

const tracks = [
  { img: yachtImg, name: "Yacht Brokerage", modules: 42, progress: 28, active: true },
  { img: villaImg, name: "Ultra-Prime Real Estate", modules: 38, progress: 0 },
  { img: jetImg, name: "Private Aviation", modules: 31, progress: 0 },
  { img: carImg, name: "Exotic Automotive", modules: 26, progress: 0 },
];

const modules = [
  { n: "01", t: "The architecture of trust with UHNW", dur: "14 min", done: true },
  { n: "02", t: "Reading a yacht spec sheet like a senior broker", dur: "22 min", done: true },
  { n: "03", t: "Charter season dynamics — Med vs Caribbean", dur: "18 min", done: true },
  { n: "04", t: "Luxury sales psychology — restraint as leverage", dur: "26 min", current: true },
  { n: "05", t: "Building a buyer mandate from a single conversation", dur: "31 min", locked: true },
  { n: "06", t: "Owner representation: the unwritten code", dur: "24 min", locked: true },
];

function Academy() {
  return (
    <AppShell>
      <div className="mb-10 animate-fade-up">
        <div className="text-[10px] tracking-[0.34em] text-primary/80 mb-2">ACADEMY</div>
        <h1 className="font-serif text-4xl sm:text-5xl">
          Become an insider — <span className="italic text-gold-gradient">methodically.</span>
        </h1>
      </div>

      <SectionHeading eyebrow="TRACKS" title="Industry curricula" />
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-14">
        {tracks.map((t) => (
          <div
            key={t.name}
            className={`glass rounded-xl overflow-hidden group cursor-pointer ${
              t.active ? "ring-gold" : ""
            }`}
          >
            <div className="relative h-40 overflow-hidden">
              <img
                src={t.img}
                alt={t.name}
                className="h-full w-full object-cover opacity-70 group-hover:opacity-100 group-hover:scale-105 transition-all duration-700"
                loading="lazy"
                width={800}
                height={600}
              />
              <div className="absolute inset-0 bg-gradient-to-t from-card to-transparent" />
            </div>
            <div className="p-5">
              <div className="font-serif text-lg">{t.name}</div>
              <div className="mt-1 text-xs text-muted-foreground">{t.modules} modules</div>
              <div className="mt-4 h-1 bg-secondary rounded-full overflow-hidden">
                <div
                  className="h-full bg-[var(--gradient-gold)]"
                  style={{ width: `${(t.progress / t.modules) * 100}%` }}
                />
              </div>
              <div className="mt-2 text-[11px] text-muted-foreground font-mono">
                {t.progress}/{t.modules} complete
              </div>
            </div>
          </div>
        ))}
      </div>

      <SectionHeading
        eyebrow="ACTIVE TRACK · YACHT BROKERAGE"
        title="Continue your immersion"
      />
      <div className="glass rounded-xl divide-y divide-border/60">
        {modules.map((m) => (
          <div
            key={m.n}
            className={`flex items-center gap-5 p-5 transition-colors ${
              m.locked ? "opacity-50" : "hover:bg-secondary/30 cursor-pointer"
            } ${m.current ? "bg-secondary/40" : ""}`}
          >
            <div className="font-mono text-xs text-muted-foreground w-8">{m.n}</div>
            <div className="flex-1">
              <div className="text-foreground">{m.t}</div>
              <div className="text-[11px] text-muted-foreground mt-0.5 font-mono">{m.dur}</div>
            </div>
            {m.done && (
              <span className="text-[10px] tracking-[0.3em] text-primary/80">COMPLETED</span>
            )}
            {m.current && (
              <button className="inline-flex items-center gap-2 bg-[var(--gradient-gold)] text-primary-foreground rounded-full px-4 py-2 text-xs">
                <Play className="h-3 w-3" /> Continue
              </button>
            )}
            {m.locked && <Lock className="h-4 w-4 text-muted-foreground" />}
          </div>
        ))}
      </div>

      <div className="glass rounded-xl mt-8 p-6 flex items-start gap-4">
        <Sparkles className="h-5 w-5 text-primary mt-0.5" />
        <div>
          <div className="font-serif text-lg">AI tutor for this module</div>
          <p className="text-sm text-muted-foreground mt-1 max-w-2xl">
            Drill the concepts of "restraint as leverage" through a live role-play
            with AURUM — playing the part of a skeptical senior owner.
          </p>
          <button className="mt-3 text-sm text-primary hover:underline">
            Begin role-play →
          </button>
        </div>
      </div>
    </AppShell>
  );
}
